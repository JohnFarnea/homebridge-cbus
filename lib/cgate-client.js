...
CBusClient.prototype.setLabel = function (netId, text, callback) {
	const cmd = `lighting label //${netId.project}/${netId.network}/${netId.application} 1 ${netId.group} - text ${text}`;
	this._sendMessage(cmd, callback);
}
...
CBusClient.prototype._sendMessage = function (message, callback) {
	...
	console.log("Sending CBUS command .. " + request.raw);
